...
const ChurchApp = {
    ...
    db: {
        ...
        events: [
            ...
            { id: 'e2', branchId: 'b2', title: 'Kawangware Community Outreach', description: 'Food, clothing and medical support for families around the Kawangware branch.', date: '2026-07-25', time: '09:00', rolesRequired: ['Greeting', 'First Aid', 'Security'], volunteersSignedUp: ['m6'] },
                ...
                ? upcoming.map((e) => {
                    ...
                    return `<li class="upcoming-event">
                        ...
                        <div class="event-info">
                            ...
                            <div class="event-branch">${esc(roles)}</div>
                        </div>
                    </li>`;
                }).join('')
   ...
   // so the trend graphs keep data across reloads.
        ...
        container.innerHTML = groups.map(g => {
            ...
            return `<div class="mobile-group-card">
                <div class="mobile-group-info">
                    ...
                    <span>${esc(g.schedule || 'TBD')} - ${(g.memberIds || []).length} members</span>
                </div>
    ...
    logPledgePayment(member, amount, method) {
        const date = new Date().toISOString().split('T')[0];
        const branchObj = this.db.branches.find(b => b.id === member.branchId);
        const newTx = {
            id: `t_${Date.now()}`,
    ...
    renderGuestContributions(branchId, filteredTx) {
        ...
        if (!guestTx.length) {
            tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: #9ca3af; padding: 30px;">No guest contributions logged yet.</td></tr>`;
        }
    ...
    campaignCardHtml(c, inScope, canManage) {
    ...
    renderGivingInsights(branchId) {
        ...
        const campaigns = (this.db.campaigns || []).filter(c => inScope(c.branchId) || (branchId === 'global'));
        const campaignCards = campaigns.map((c) => this.campaignCardHtml(c, inScope, canManage)).join('');
        ...
        const insightsHeader = canManage ? `<div class="giving-insights-head">
            <span style="font-family: var(--font-header); font-weight:700; color:var(--text-primary);">Projects &amp; Pledge Campaigns</span>
            <button type="button" class="btn btn-secondary btn-sm" onclick="ChurchApp.openNewProject()">+ New Project</button>
        </div>` : '';
    ...
    renderFinancialSummary(branchId, filteredTx) {
        ...
        const money = (n) => window.money(n, { decimals: 0 });
        const card = (label, value) => `<div class="card-glass campaign-card summary-stat-card">
        ...
        const pledgeCard = `<div class="card-glass campaign-card summary-stat-card">
        ...
        el.innerHTML = '<div class="giving-insights-head"><span style="font-family: var(--font-header); font-weight:700; color:var(--text-primary);">Financial Summary</span></div>' +
            card('Total Contributions', money(total)) + card('Contributions Filed', String(tx.length)) +
            byFund.map(f => f.fund === 'Pledge' ? pledgeCard : card(f.fund, money(f.amount))).join('');
    },
   ...
   printMemberProfile(memberId) {
    ...
    handleRecordTransaction() {
        ...
        if (memberId !== 'anonymous') {
            ...
            if (memberObj) {
                ...
                // pledge, so the balance in the form stays in sync.
                if (category === 'Pledge') {
            ...
            <div class="modal-card receipt-card" role="dialog" aria-modal="true" aria-labelledby="receipt-modal-title">
                ...
                <div class="modal-body receipt-print-area">
                    ...
                    <div class="receipt-footer">
                        ...
                        <p style="font-size:0.7rem; color:#9ca3af; margin-top:8px;">Signed electronically by the ${esc('Church Connect')} HQ admin system</p>
                    </div>
                </div>
                <div style="display: flex; gap: 8px; margin-top: 15px; justify-content: flex-end;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:12px; flex-wrap:wrap;">
                    <div style="flex:1; min-width:220px;">
                        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                            <strong style="color: #93c5fd; font-size: 0.95rem;">${esc(entry.role)}</strong>
    ...
    renderMobilePreview() {
        ...
        const subviews = ['mobile-home', 'mobile-sermons', 'mobile-bible', 'mobile-give', 'mobile-projects', 'mobile-serve', 'mobile-chat', 'mobile-profile'];
        subviews.forEach(sv => {
    ...
    _renderBibleChapter() {
        ...
        if (this._kjvData) {
            ...
            if (!book || !Array.isArray(verses)) {
                verseContainer.innerHTML = `<div style="text-align:center; color:var(--mob-sub); padding:15px; font-size:0.75rem;">Select a book to begin reading.</div>`;
            } else {
                let shown = 0;
                verses.forEach((text, i) => {
                    const refStr = book.name + ' ' + chapter + ':' + (i + 1);
                    if (searchInput && !this.kjvClean(text).toLowerCase().includes(searchInput) && !refStr.toLowerCase().includes(searchInput)) return;
                    shown++;
                    const p = document.createElement('div');
                    p.className = 'mobile-bible-verse-item';
                    p.innerHTML = `
                        <strong style="font-size:0.8rem; color:var(--mob-accent-2); display:block;">${esc(book.name)} ${chapter}:${i + 1} (KJV)</strong>
                        <p style="font-size:0.8rem; color:var(--text-secondary); margin-top:2px;">"${this.kjvDisplay(text)}"</p>
                    `;
                    verseContainer.appendChild(p);
                });
                if (shown === 0) {
                    verseContainer.innerHTML = `<div style="text-align:center; color:var(--mob-sub); padding:15px; font-size:0.75rem;">No verses found matching query.</div>`;
                }
            }
        } else {
            ...
            filtered.forEach((v) => {
                ...
                p.className = 'mobile-bible-verse-item';
                p.innerHTML = `
                    <strong style="font-size:0.8rem; color:var(--mob-accent-2); display:block;">${esc(v.ref)} (${esc(this.session.bibleVersion)})</strong>
    ...
    renderMobileGive() {
        ...
        if (onBehalfSel) {
            ...
            onBehalfSel.innerHTML = `<option value="">${esc(selfLabel)}</option>` +
                (member ? this.effectiveFamilyMembers(member).map((fm) => `<option value="${esc(fm.name)}">${esc(fm.name)}</option>`).join('') : '');
        }
        ...
        const giveCategorySel = document.getElementById('mobile-giving-category');
        if (giveCategorySel) giveCategorySel.onchange = () => this.renderMobileGivePledge();
        this.renderMobileGivePledge();
    },
    ...
    renderMobileGivePledge() {
        ...
        const member = this.db.members.find(m => m.id === this.simulatedMemberId());
        const p = this.memberPledge(member);
        const payments = (this.db.transactions || []).filter(t => t.memberId === member.id && String(t.category || '').toLowerCase() === 'pledge');
        ...
        pledgeEl.innerHTML = `
            <div class="mobile-pledge-card">
                ...
                ${payments.length ? `<div style="font-size:0.68rem; color:var(--mob-sub); margin-top:8px;">Paid via: ${payMethods.map(mth => `<span class="category-pill category-pledge" style="margin-right:4px;">${esc(mth)}</span>`).join('')}</div>` : '<div style="font-size:0.68rem; color:var(--mob-sub); margin-top:8px;">No pledge payments yet.</div>'}` : ''}
    ...
    handleMobileGiving() {
        ...
        const onBehalfOf = document.getElementById('mobile-giving-onbehalf') ? document.getElementById('mobile-giving-onbehalf').value : '';
